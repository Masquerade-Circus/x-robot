...
import { createTrackedMachineAdapter, readMachineState } from "../src/internal";

describe("@x-robot/vue internal tracked adapter", () => {
  ...
  it("tracks invokeAfter cancellation and cleanup aliases", (done) => {
    const updates: any[] = [];
    const checkoutMachine = machine(
      "Checkout",
      init(initial("idle")),
  ...
  it("serializes fatal state after async completion", async () => {
    const updates: any[] = [];
    const checkoutMachine = machine(
      "Async fatal checkout",
      init(initial("idle")),
      state("idle", transition("submit", "saving")),
      state(
        "saving",
    ...
    await tracked.invoke("submit");

    expect(checkoutMachine.current).toBe("fatal");
    expect(updates).toHaveLength(1);
    expect(updates[0]).toMatchObject({
      action: { kind: "invoke", type: "submit" },
      snapshot: {
        current: "fatal",
        isAsync: true,
        fatal: {
